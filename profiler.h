...
namespace profiler
{
	...
	public:
		...
		*/
		...
		struct implBase { virtual ~implBase() = default; };
	private:
		void beginImpl(const std::string& label, const std::thread::id& tid);
		void endImpl(const std::string& label, const std::thread::id& tid);
